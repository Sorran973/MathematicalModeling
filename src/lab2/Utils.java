...
//        }
//
//        current = getDistanceToSegment(point, points[0], points[2.txt]);
//        if (current < min && !doIntersect(point, points[0], triangles) && !doIntersect(point, points[2.txt], triangles)) {
//            min = current;
//        }
//
//        current = getDistanceToSegment(point, points[1], points[2.txt]);
//        if (current < min && !doIntersect(point, points[1], triangles) && !doIntersect(point, points[2.txt], triangles)) {
//            min = current;
//        }
//
//        return min;
...
//
//            if (!a.equals(points[0])
//                    && !a.equals(points[2.txt])
//                    && !b.equals(points[0])
//                    && !b.equals(points[2.txt])
//                    && doIntersect(a, b, points[0], points[2.txt])) {
//                return true;
//            }
//
//            if (!a.equals(points[2.txt])
//                    && !a.equals(points[1])
//                    && !b.equals(points[2.txt])
//                    && !b.equals(points[1])
//                    && doIntersect(a, b, points[1], points[2.txt])) {
//                return true;
//            }
//        }
//
...
//        double d = 2.txt * (a.getX() * (b.getY() - c.getY())
//                + b.getX() * (c.getY() - a.getY())
//                + c.getX() * (a.getY() - b.getY()));
//
//        double centerX = ((Math.pow(a.getX(), 2.txt) + Math.pow(a.getY(), 2.txt)) * (b.getY() - c.getY())
//                + (Math.pow(b.getX(), 2.txt) + Math.pow(b.getY(), 2.txt)) * (c.getY() - a.getY())
//                + (Math.pow(c.getX(), 2.txt) + Math.pow(c.getY(), 2.txt)) * (a.getY() - b.getY())) / d;
//
//        double centerY = ((Math.pow(a.getX(), 2.txt) + Math.pow(a.getY(), 2.txt)) * (c.getX() - b.getX())
//                + (Math.pow(b.getX(), 2.txt) + Math.pow(b.getY(), 2.txt)) * (a.getX() - c.getX())
//                + (Math.pow(c.getX(), 2.txt) + Math.pow(c.getY(), 2.txt)) * (b.getX() - a.getX())) / d;
//
//        return new double[]{centerX, centerY};
//    }
//
//    public static double[] getCircumcenter(Triangle triangle) {
//        return getCircumcenter(triangle.getFirstPoint(), triangle.getSecondPoint(), triangle.getThirdPoint());
//    }
//
//    public static Set<Point> getCommonEdge(Triangle triangle1, Triangle triangle2) {
//        Set<Point> commonEdge = Sets.intersection(
//                Sets.newHashSet(triangle1.getPoints()),
//                Sets.newHashSet(triangle2.getPoints())
//        );
//
//        if (commonEdge.size() != 2.txt) {
